@testset "Test Influence Matrix" begin
    ...
    @test(abs(IMBP.im_distance(rand_im, rand_im)) < 1e-10)
    ...
    @test(abs(IMBP.im_distance(rand_im_clone2, rand_im)) < 1e-5)

    ims = Dict{IMBP.IMID, IMBP.IM{Array{ComplexF64, 4}}}(
        IMBP.IMID(1, true, (1, 0)) => IMBP._random_im(IM{Array{ComplexF64, 4}}, rng, 3, 21),
        IMBP.IMID(2, false, (0, 2)) => IMBP._random_im(IM{Array{ComplexF64, 4}}, rng, 3, 21),
        IMBP.IMID(4, false, (0, 3)) => IMBP._random_im(IM{Array{ComplexF64, 4}}, rng, 3, 20),
    )

    kernels = Dict{IMBP.KernelID, Vector{Node{Array{ComplexF64, 4}}}}(
        IMBP.KernelID(3, true, (0, 4)) => [Node(randn(rng, ComplexF64, 4, 4, 4, 4), :first_inp, :second_inp, :first_out, :second_out) for _ in 1:21],
    )

    one_qubit_gates = [Node(randn(rng, ComplexF64, 4, 4), :pinp, :pout) for _ in 1:21]
    ...
    new_im, _ = IMBP.contract(equation, ims, kernels, one_qubit_gates, initial_state, 1e-20, 21)
