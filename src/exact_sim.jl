...
@inline insert_zero(index::UInt, mask::UInt, flipped_mask::UInt)::UInt = ((index & mask) << 1) | (index & flipped_mask)

@inline get_mask(pos::UInt)::UInt = typemax(UInt) << (pos - 1)

@inline flip_mask(mask::UInt)::UInt = ~mask

@inline get_stride(pos::UInt)::UInt = 1 << (pos - 1)
...
mutable struct QSim{T<:Real}
    ...
    function QSim(::Type{T}, qubits_number::Int) where {T<:Real}
        ...
        new{T}(state, convert(UInt, qubits_number))
    end
end
...
function apply_two_qubit_gate!(qsim::QSim{T}, gate::Matrix{Complex{T}}, pos1::Int, pos2::Int) where {T<:Real}
    ...
    @assert pos2 >= 1
    apply_two_qubit_gate!(qsim, gate, convert(UInt, pos1), convert(UInt, pos2))
end
...
function apply_two_qubit_gate!(
    ...
    state = qsim.state
    @inbounds for i in zero(UInt):UInt(size - 1)
...
function apply_one_qubit_gate!(qsim::QSim{T}, gate::Matrix{Complex{T}}, pos::Int) where {T<:Real}
    ...
    @assert pos >= 1
    apply_one_qubit_gate!(qsim, gate, convert(UInt, pos))
end

function apply_one_qubit_gate!(qsim::QSim{T}, gate::Matrix{Complex{T}}, pos::UInt) where {T<:Real}
    mask = get_mask(pos)
...
function get_one_qubit_dens(qsim::QSim{T}, pos::Int) where {T<:Real}
    @assert pos <= qsim.qubits_number
    @assert pos >= 1
    get_one_qubit_dens(qsim, convert(UInt, pos))
end

function get_one_qubit_dens(qsim::QSim{T}, pos::UInt) where {T<:Real}
